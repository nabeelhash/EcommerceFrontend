...
const CreateCategory = () => {
    ...
    const newCategory = async function (e) {
        ...
        try {
            console.log('click')
            let response = await fetch('https://ecommerce-backend-pi-neon.vercel.app/category', {
                method: 'POST',
                headers: { "Content-Type": "application/json" },
                credentials: 'include',
    ...
    const updateCategory = async function () {
        try {
            ...
            let response = await fetch(`https://ecommerce-backend-pi-neon.vercel.app/updateCategory/${updateId}`, {
                method: 'PATCH',
                headers: { "Content-Type": "application/json" },
                credentials: 'include',
    ...
    const handleDelete =async function(id){
        try {
            console.log(updateId)
            let response = await fetch(`https://ecommerce-backend-pi-neon.vercel.app/deleteCategory/${id}`, {
                method: 'delete',
                headers: { "Content-Type": "application/json" },
                credentials: 'include',
            })
    ...
    return (
        <div className='w-[80%] flex flex-col  justify-center items-center'>
            <div className='w-[70%]  m-auto '>
                <div class="flex gap-1  items-center justify-center mb-5">
                    ...
                    <button type="submit" onClick={newCategory} class=" btn bg-gradient-to-r from-blue-500 to-purple-500 text-white transform transition-transform duration-200 hover:scale-105 py-2 m-2">Create Category</button>
                </div>
                <table class="table ">
                    <thead>
                        <tr>
